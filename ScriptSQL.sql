...
SET IDENTITY_INSERT Customer ON  (se a  coluna estiver como off) 
...
SELECT FirstName, LastName, City   (Especifico)
  FROM Customer
GO

SELECT *            (Busca toda informação)
  FROM Customer
GO

SELECT Id, FirstName, LastName, City, Country, Phone
  FROM Customer
 WHERE Country = 'Sweden'        (filtra o pais)
GO

SELECT CompanyName, ContactName, City, Country
  FROM Supplier
 ORDER BY CompanyName         (Ordenar Ascendente (A,B,C)            
GO 

SELECT CompanyName, ContactName, City, Country
  FROM Supplier
 ORDER BY CompanyName DESC         (Ordenar Decresente)
GO

SELECT FirstName, LastName, City, Country
  FROM Customer
 ORDER BY Country, City    (Ordenar pelo pais,cidade)
GO

-- SELECT TOP
-- Problem: List the top 10 most expensive products ordered by price

SELECT TOP 10 Id, ProductName, UnitPrice, Package  (Os 10 primeiros)
  FROM Product
...
SELECT DISTINCT Country  (Traz apenas uma vez o pais)
  FROM Supplier
ORDER BY COUNTRY
...
SELECT MAX(TotalAmount)             (maior preço)
  FROM [Order]
 WHERE YEAR(OrderDate) = 2014       (Vai verificar so o ano)
----------------------------------------------------------------------------------------

-- SQL SELECT COUNT, SUM, and AVG     

SELECT COUNT(Id)      (conta a quantidade)
  FROM Customer

SELECT SUM(TotalAmount)    (soma todos os pedidos)
  FROM [Order]
 WHERE YEAR(OrderDate) = 2013  (todos pedidos 2013)

SELECT AVG(TotalAmount) as media  (Media do pedido)
  FROM [Order]
----------------------------------------------------------------------------------------
...
SELECT Id, OrderDate, CustomerId, TotalAmount
  FROM [Order]
 WHERE NOT (TotalAmount >= 500 AND TotalAmount <= 15000) (Negação)
 ORDER BY TotalAmount DESC
---------------------------------------------------------------------------------------
-- SQL WHERE BETWEEN  (entre)
...
SELECT Id, CompanyName, Phone, Fax 
  FROM Supplier
 WHERE Fax IS NULL   (Campo nulo)

-- Problem: List all suppliers that do have a fax number
SELECT Id, CompanyName, Phone, Fax 
  FROM Supplier
 WHERE Fax IS NOT NULL   (campo que nao seja nulo)
---------------------------------------------------------------------------------------
-- SQL GROUP BY
...
SELECT 'Customer' As Type, 
       FirstName + ' ' + LastName AS ContactName,   (Junta duas tabelas uma embaixo da outra)
       City, Country, Phone
  FROM Customer
UNION
SELECT 'Supplier', 
...
SELECT  Country , COUNT(Id) as qt
 ...
 WHERE Country <> 'USA'
 GROUP BY Country
HAVING COUNT(Id) >= 9   (Faz um filtro de um filtro)
 ORDER BY COUNT(Id) DESC
